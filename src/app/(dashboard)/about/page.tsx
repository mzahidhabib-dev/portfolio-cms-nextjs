...
import { 
  Save, Upload, Type, User, 
  GraduationCap, Briefcase, Rocket, MousePointer2, 
  X, RotateCcw 
} from "lucide-react";
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 pb-24">
        ...
        <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm">Tell your story and showcase your achievements.</p>
          </div>
          <Button 
            variant="outline" 
            size="sm" 
        ...
        <div className="grid gap-6 lg:grid-cols-3">
            ...
            <div className="rounded-xl border bg-card p-6 shadow-sm space-y-4">
              ...
              <FormField control={form.control} name="description" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Textarea className="min-h-[160px] resize-none" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
            </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                <FormField control={form.control} name="education" render={({ field }) => (
                  <FormItem className="text-center p-4 rounded-lg bg-muted/20 border">
                    ...
                    <FormControl><Input className="text-center font-bold text-lg h-8 bg-background" {...field} /></FormControl>
                    <p className="text-[10px] text-muted-foreground uppercase font-bold mt-1">Education</p>
                  </FormItem>
                )} />
                <FormField control={form.control} name="experience" render={({ field }) => (
                  <FormItem className="text-center p-4 rounded-lg bg-muted/20 border">
                    ...
                    <FormControl><Input className="text-center font-bold text-lg h-8 bg-background" {...field} /></FormControl>
                    <p className="text-[10px] text-muted-foreground uppercase font-bold mt-1">Experience</p>
                  </FormItem>
                )} />
                <FormField control={form.control} name="projects" render={({ field }) => (
                  <FormItem className="text-center p-4 rounded-lg bg-muted/20 border">
                    ...
                    <FormControl><Input className="text-center font-bold text-lg h-8 bg-background" {...field} /></FormControl>
                    <p className="text-[10px] text-muted-foreground uppercase font-bold mt-1">Projects</p>
                  </FormItem>
                )} />
              </div>
            </div>
          ...
          <div className="space-y-6">
              ...
              >
                {imgPreview ? (
                  <div className="relative group">
                    <img src={imgPreview} alt="About Me" className="max-h-48 object-contain rounded-lg shadow-md" />
                    <button type="button" onClick={(e) => { e.stopPropagation(); setImgPreview(null); }} className="absolute -top-2 -right-2 bg-destructive text-white rounded-full p-1"><X className="size-3" /></button>
                  </div>
                ) : (
                  <div className="text-center">
                    <Upload className="size-6 text-muted-foreground mx-auto mb-2" />
                    <p className="text-xs text-muted-foreground">Click to upload About Image</p>
                  </div>
                )}
            ...
            <div className="rounded-xl border bg-card p-6 shadow-sm space-y-4">
              ...
              <FormField control={form.control} name="btnText" render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-xs">Button Label</FormLabel>
                  <FormControl><Input placeholder="Learn More" {...field} /></FormControl>
                </FormItem>
              )} />
              <FormField control={form.control} name="btnLink" render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-xs">Redirect Link (URL)</FormLabel>
                  <FormControl><Input placeholder="/about-details" {...field} /></FormControl>
                </FormItem>
              )} />
            </div>
          </div>
        ...
        <div className="fixed bottom-8 right-8">
          <Button type="submit" size="lg" className="rounded-full shadow-2xl gap-2 px-10 h-14 font-semibold">
