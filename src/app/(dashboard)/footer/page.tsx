...
export default function FooterManager() {
  ...
  const onSave = (data: z.infer<typeof footerSchema>) => {
    ...
    <div className="space-y-6 pb-24 px-4 sm:px-0 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b pb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Manage global identity, contact info, and social links.</p>
        </div>
        <Button 
          variant="outline" 
          size="sm" 
          onClick={() => { if(confirm("Reset all footer settings?")) form.reset(defaultValues); }} 
        ...
        <div className="grid gap-6 lg:grid-cols-12">
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="p-6 border rounded-2xl bg-card space-y-5 shadow-sm">
              <h3 className="font-bold flex items-center gap-2 text-primary uppercase text-xs tracking-widest"><Globe className="size-4" /> Identity & Links</h3>
              <FormField control={form.control} name="tagline" render={({ field }) => (
                <FormItem><FormLabel className="text-xs">Tagline</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>
              )} />
              <FormField control={form.control} name="quickLinksTitle" render={({ field }) => (
                <FormItem><FormLabel className="text-xs">Quick Links Title</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>
              )} />
            </div>

            <div className="p-6 border rounded-2xl bg-card space-y-5 shadow-sm">
              <h3 className="font-bold flex items-center gap-2 text-primary uppercase text-xs tracking-widest"><Mail className="size-4" /> Contact Details</h3>
              <FormField control={form.control} name="email" render={({ field }) => (
                <FormItem><FormLabel className="text-xs">Email Address</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>
              )} />
              <FormField control={form.control} name="phone" render={({ field }) => (
                <FormItem><FormLabel className="text-xs">Phone Number</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>
              )} />
            </div>

            <div className="p-6 border rounded-2xl bg-card space-y-5 shadow-sm">
              <h3 className="font-bold flex items-center gap-2 text-primary uppercase text-xs tracking-widest"><ShieldCheck className="size-4" /> Legal Bar</h3>
              <FormField control={form.control} name="copyrightText" render={({ field }) => (
                <FormItem><FormLabel className="text-xs">Copyright Text</FormLabel><FormControl><Input {...field} /></FormControl></FormItem>
              )} />
            </div>
          </div>
            ...
            <div className="p-6 border rounded-2xl bg-card shadow-sm min-h-full space-y-6">
              <div className="flex items-center justify-between border-b pb-4">
                <div className="space-y-1">
                  <h3 className="font-bold flex items-center gap-2 text-primary uppercase text-xs tracking-widest"><LinkIcon className="size-4" /> Social Links</h3>
                  <p className="text-[10px] text-muted-foreground">Add your profiles with custom icons.</p>
                </div>
                <Button 
                  type="button" 
                  size="sm" 
                  onClick={() => append({ id: Date.now().toString(), platform: "", url: "", logo: "" })}
              ...
              <div className="space-y-4">
                {fields.map((field, index) => {
                  ...
                  return (
                    <div key={field.id} className="group relative p-4 rounded-xl border bg-muted/10 hover:bg-muted/30 transition-all border-dashed hover:border-solid hover:border-primary/30">
                      <div className="flex gap-4 items-start">
                        ...
                        <div className="space-y-1 text-center">
                          ...
                          >
                            {currentLogo ? (
                              <img src={currentLogo} className="w-full h-full object-contain p-1.5" alt="icon" />
                            ) : (
                              <Upload className="size-4 text-muted-foreground opacity-30" />
                            )}
                          </div>
                          <span className="text-[9px] font-bold text-muted-foreground uppercase">Icon</span>
                          <input 
                        ...
                        <div className="flex-1 space-y-2">
                          <Input 
                            ...
                            {...form.register(`socials.${index}.platform` as const)} 
                          />
                          <Input 
                          ...
                          {deleteConfirmId === field.id ? (
                            <div className="flex flex-col gap-1 animate-in zoom-in">
                              <Button 
                                ...
                                onClick={() => { remove(index); setDeleteConfirmId(null); toast.error(`${platformName || 'Social'} removed`); }}
                              >
                ...
                {fields.length === 0 && (
                  <div className="py-12 border-2 border-dashed rounded-xl text-center opacity-30">
                    ...
                    <p className="text-xs font-bold uppercase tracking-widest">No social links configured</p>
                  </div>
                )}
              </div>
              ...
              {/* SAVE BUTTON */}
              <div className="pt-4">
                <Button onClick={form.handleSubmit(onSave)} size="lg" className="w-full gap-3 font-bold h-14 shadow-xl rounded-full">
