...
import { 
  Save, Upload, Type, FileText, MousePointer2, 
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 pb-24">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="rounded-xl border bg-card p-6 shadow-sm space-y-6">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="p-4 rounded-lg bg-muted/20 border space-y-3">
                  <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">CV Download</p>
                  <FormField
                    control={form.control}
                    name="cvText"
                    render={({ field }) => (
                  ...
                  />
                  <Input type="file" accept=".pdf" className="h-8 text-[10px] px-1 py-1" />
                </div>

                <div className="p-4 rounded-lg bg-muted/20 border space-y-3">
                  <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Contact Link</p>
          ...
          <div className="rounded-xl border bg-card p-6 shadow-sm h-fit space-y-6">
            ...
            <div className="space-y-2">
              <Label className="text-sm font-medium">Main Profile Picture</Label>
              <div 
              ...
              >
                {mainImg ? (
                  <div className="relative group">
                    <img src={mainImg} alt="Preview" className="max-h-32 object-contain rounded-md" />
                    <button type="button" onClick={(e) => { e.stopPropagation(); setMainImg(null); }} className="absolute -top-2 -right-2 bg-destructive text-white rounded-full p-1"><X className="size-3" /></button>
                  </div>
                ) : (
                  <div className="flex flex-col items-center"><Upload className="size-5 mb-2 text-muted-foreground" /><span className="text-xs text-muted-foreground">Upload Profile</span></div>
                )}
                <input type="file" ref={mainFileRef} className="hidden" accept="image/*" onChange={(e) => handleImage(e, setMainImg)} />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              >
                {secImg ? (
                  <div className="relative group">
                    <img src={secImg} alt="Preview" className="max-h-24 object-contain rounded-md" />
                    <button type="button" onClick={(e) => { e.stopPropagation(); setSecImg(null); }} className="absolute -top-2 -right-2 bg-destructive text-white rounded-full p-1"><X className="size-3" /></button>
                  </div>
                ) : (
                  <div className="flex flex-col items-center"><Upload className="size-5 mb-2 text-muted-foreground" /><span className="text-xs text-muted-foreground">Upload Background</span></div>
                )}
                <input type="file" ref={secFileRef} className="hidden" accept="image/*" onChange={(e) => handleImage(e, setSecImg)} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed bottom-8 right-8">
          <Button type="submit" size="lg" className="rounded-full shadow-2xl gap-2 px-10 h-14 font-semibold shadow-primary/20">
